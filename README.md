...
Below are some visual examples generated by the program for different space dimensions.

### 2D — Lines on a Plane
When several lines are in general position (no parallels, no triple intersections),  
they divide the plane into regions according to the formula:  
R(n, 2) = 1 + n + n(n − 1)/2.

![2D Example](lines_example.png)
> For n = 3 lines, the plane is divided into 7 regions (R = 7).

---

### 3D — Planes in Space
In 3-dimensional space, each plane intersects others along lines,  
and together they form a complex arrangement of semi-transparent layers.

![3D Example](planes_example.png)
> For n = 4 planes, the space is divided into 15 regions (R = 15).

---

### 💡 Interactive Visualization
The program allows users to:
- Choose the space dimension (1D / 2D / 3D);
- Select the number of hyperplanes;
- View the geometric configuration and the corresponding number of regions.

All figures above were generated using Python’s **NumPy** and **Matplotlib** libraries.
